...
var simWidth = 350, simHeight = 350;
...
function drawSystem(planetName, SystemData) {
	...
	d3.csv("./given-data/oec.csv", function (error, data) {
		...
		var starTemp, starRad, luminosity, absLuminosity, hzRadin, hzRadout, type, discMethod, hzCalculated = false;
		SystemData.forEach(planet => {
			findPlanetByName(planet.PlanetIdentifier, masterData, function (d) {
				discMethod = d.DiscoveryMethod, distSun = +d.DistFromSunParsec, starTemp = +d.HostStarTempK, starRad = +d.HostStarRadiusSlrRad * 696000, type = +d.TypeFlag
				if (+d.SemiMajorAxisAU < 0.1) {
					isSmallSystem = true;
					planets.push({ color: tempColor(+d.SurfaceTempK), rad: radScale(+d.RadiusJpt), dist: distScaleLow(+d.SemiMajorAxisAU * 5) + 10, speed: speedScale(calculateSpeed(+d.PeriodDays)), angle: Math.random() * 360 })
				} else {
					planets.push({ color: tempColor(+d.SurfaceTempK), rad: radScale(+d.RadiusJpt), dist: distScale(+d.SemiMajorAxisAU * 5), speed: speedScale(calculateSpeed(+d.PeriodDays)), angle: Math.random() * 360 })
				}
			})
		});
		...
		if (!+starTemp == 0 && !+starRad == 0) {
			...
			sim
				...
				.append("circle")
				.attr("r", handleNaN(isSmallSystem ? distScalehz(hzRadin) + 10 : distScale(hzRadin), 0))
				.attr("fill", "none").attr("stroke", "#75b401").attr("opacity", 0.3)
				.attr("stroke-width",
					handleNaN(isSmallSystem ? distScalehz(Math.abs(hzRadin - hzRadout)) + 10 : distScale(Math.abs(hzRadin - hzRadout))), 0)
				.attr("cx", simWidth / 2)
				.attr("cy", simHeight / 2)
		...
		switch (type) {
			case 1: var t = sim.append("text").attr("x", 100).attr("y", 100).attr("opacity", 0.5)
				t.append("tspan").attr("x", 70).attr("dy", "1.2em").text("This is a P-Type Binary System.")
				t.append("tspan").attr("x", 30).attr("dy", "1.2em").text("Research says that 50-60% Binary Systems")
				t.append("tspan").attr("x", 10).attr("dy", "1.2em").text("support habitable zone in selected orbital ranges.");
				t.append("tspan").attr("x", 30).attr("dy", "1.2em").text("(ps..the model might not be accurate :/)"); break;

			case 2: var t = sim.append("text").attr("x", 100).attr("y", 100).attr("opacity", 0.5)
				t.append("tspan").attr("x", 70).attr("dy", "1.2em").text("This is a S-Type Binary System.")
				t.append("tspan").attr("x", 30).attr("dy", "1.2em").text("Research says that 50-60% Binary Systems")
				t.append("tspan").attr("x", 10).attr("dy", "1.2em").text("support habitable zone in selected orbital ranges.");
				t.append("tspan").attr("x", 30).attr("dy", "1.2em").text("(ps..the model might not be accurate :/)"); break;

			case 3: var t = sim.append("text").attr("x", 100).attr("y", 100).attr("opacity", 0.5)
				t.append("tspan").attr("x", 70).attr("dy", "1.2em").text("404 not found :(")
				t.append("tspan").attr("x", 30).attr("dy", "1.2em").text("This might be a orphan star or")
				t.append("tspan").attr("x", 10).attr("dy", "1.2em").text("data maynot be avialable.")
				t.append("tspan").attr("x", 30).attr("dy", "1.2em").text("(ps..the model might not be accurate :/)"); break;
		}
		...
		simCont.selectAll("g.planet").data(planets).enter().append("g")
			.attr("class", "planet").each(function (d, i) {
				d3.select(this).append("circle").attr("class", "planet-sim-orbit").attr("fill", "none").attr("stroke", "#dddddd").transition().duration(600).ease(d3.easeCubicInOut)
					.attr("r", handleNaN(+d.dist, handleNaN(i, 1) * 10));
				d3.select(this).append("circle").transition().ease(d3.easeCubicInOut).duration(600).attr("r", handleNaN(+d.rad, 5)).attr("cx", handleNaN(d.dist, handleNaN(i, 1) * 10))
		...
		d3.select("#system-info").select("#system-name").text(planetName)
		planetInfo.select("#planets-no").html("It has<span class = 'highlight'> " + SystemData.length + " </span>orbiting planets")
		planetInfo.select("#planets-list").text(planetArr.join(", "))
		planetInfo.select("#distance-sun").html(handleNaN(distSun, 0) == 0 ? "Distance from sun data not available" : "It is<span class = 'highlight'> "
			+ distSun + "</span> parsecs from Sun, It would take " + 3.262 * distSun +
			" years to get here if travelled at speed of light."
		)
		planetInfo.select("#disc-method").html("Discovered by <span class = 'highlight'>" + (discMethod == "RV" ? "Radial Velocity" : discMethod) + "</span> method")
		planetInfo.select("#hz").html(hzCalculated ? "Habitable Zone calculated(green patch)" : "Habitable Zone not calculated, insufficient data available :(")
	});
}